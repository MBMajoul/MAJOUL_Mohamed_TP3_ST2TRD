...
namespace MAJOUL_Mohamed_TP3_ST2TRD
{
    public class MovieCollection
    {
        ...
        public static void Exercise1()
        {
            ...
            Console.WriteLine("Display the title of the oldest movie: {0}", Movies.OrderBy(x => x.ReleaseDate).Select(x => x.Title).FirstOrDefault());
            Console.WriteLine("Count all movies: {0}", Movies.Count());
            Console.WriteLine("Count all movies with the letter e. at least once in the title: {0}", Movies.Count(x => x.Title.ToLower().Contains('e')));
            int count = 0;
            foreach (var ttl in Movies.Select(x => x.Title.ToLower()))
            {
                count += ttl.Count(x => x == 'f');
            }
            Console.WriteLine("Count how many time the letter f is in all the titles from this list: {0}", count);
            Console.WriteLine("Display the title of the film with the higher budget: {0}", Movies.OrderByDescending(x => x.Budget).Select(x => x.Title).FirstOrDefault());
            Console.WriteLine("Display the title of the movie with the lowest box office: {0}", Movies.OrderBy(x => x.BoxOffice).Select(x => x.Title).FirstOrDefault());
            Console.WriteLine("Order the movies by reversed alphabetical order and print the first 11 of the list: {0}", string.Join(", ", Movies.Select(x => x.Title).OrderByDescending(x => x).Take(11)));
            Console.WriteLine("Count all the movies made before 1980: {0}", Movies.Where(x => x.ReleaseDate < new DateTime(1980, 01, 01)).Count());
            ...
            for (int i = 1; i < 100; i++)
            {
                foreach (var ttl in Movies.Select(x => x.Title))
                {
                    if (ttl.Length == i)
                    {
                        count++;
                ...
                if (count != 0)
                {
                    Console.WriteLine(i + " char => " + count + " films");
                    count = 0;
                }
            }
        ...
        public static void Exercise2()
        {
            ...
            Thread nThread2 = new Thread(Thread2);
            Thread nThread3 = new Thread(Thread3);
            nThread2.Start();
            nThread3.Start();
            for (int i = 0; i < 200; i++)
            {
